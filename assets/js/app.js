...
// ===== NUEVO: funciones para filtros avanzados =====
function getUniqueTypes() {
  var types = new Set();
  BUS_STATE.features.forEach(function (b) { if (b.type) types.add(b.type); });
  return Array.from(types).sort();
}
function getUniqueTechs() {
  var techs = new Set();
  BUS_STATE.features.forEach(function (b) { if (b.tech) techs.add(b.tech); });
  return Array.from(techs).sort();
}
function getYearsRange() {
  var years = new Set();
  BUS_STATE.features.forEach(function (b) { if (b.year) years.add(b.year); });
...
function initBusAdvancedFilters() {
  ...
  if (!typeSel || !techSel || !yearSel) return;

  // Llenar selects
  var types = getUniqueTypes();
  typeSel.innerHTML = '<option value="__all">Todos los tipos</option>';
  ...
  var techs = getUniqueTechs();
  techSel.innerHTML = '<option value="__all">Todas las tecnologías</option>';
  ...
  var years = getYearsRange();
  yearSel.innerHTML = '<option value="__all">Todos los años</option>';
  ...
  typeSel.addEventListener('change', function () {
    BUS_FILTERS.type = this.value;
    renderBusLayer();
    updateBusSummary();
  });
  techSel.addEventListener('change', function () {
    BUS_FILTERS.tech = this.value;
    renderBusLayer();
    updateBusSummary();
  });
  yearSel.addEventListener('change', function () {
    BUS_FILTERS.year = this.value;
    renderBusLayer();
    updateBusSummary();
  });
  if (speedMinInput) {
    speedMinInput.addEventListener('input', function () {
      BUS_FILTERS.speedMin = parseFloat(this.value) || 0;
      renderBusLayer();
      updateBusSummary();
    });
  }
  if (speedMaxInput) {
    speedMaxInput.addEventListener('input', function () {
      BUS_FILTERS.speedMax = parseFloat(this.value) || 999;
      renderBusLayer();
      updateBusSummary();
    });
  }
...
function filteredBuses() {
  ...
  return BUS_STATE.features.filter(function (bus) {
    ...
    if (BUS_FILTERS.year !== '__all' && bus.year !== parseInt(BUS_FILTERS.year, 10)) return false;
    var speed = bus.speed || 0;
...
function busPopupHtml(bus) {
  ...
  return '<div class="bus-popup">' +
    ...
    '<div><dt>Tipo de bus</dt><dd>' + esc(bus.type || 'No informado') + '</dd></div>' +
    '<div><dt>Modelo</dt><dd>' + esc(vehicle.model || 'No informado') + '</dd></div>' +
    '<div><dt>Año</dt><dd>' + esc(bus.year || 'No informado') + '</dd></div>' +
    '<div><dt>Tecnología / Combustible</dt><dd>' + esc(bus.tech || 'No informado') + '</dd></div>' +
...
function updateBusSummary() {
  var total = BUS_STATE.features.length;
  var filtered = filteredBuses();
  ...
  filtered.forEach(function (b) {
    if (b.operatorName) operators[b.operatorName] = (operators[b.operatorName] || 0) + 1;
    if (b.speed !== null && isFinite(b.speed)) speeds.push(b.speed);
    if (b.type) types[b.type] = (types[b.type] || 0) + 1;
    if (b.tech) techs[b.tech] = (techs[b.tech] || 0) + 1;
  });
  var avgSpeed = speeds.length ? Math.round(speeds.reduce(function (a, b) { return a + b; }, 0) / speeds.length) : null;
  var maxSpeed = speeds.length ? Math.max.apply(null, speeds) : null;
  var minSpeed = speeds.length ? Math.min.apply(null, speeds) : null;

  var container = document.getElementById('bus-summary');
  if (!container) return;
  container.innerHTML = '';
  // Tarjetas principales
  var cards = [
    { label: 'Total buses', value: total, sub: 'informados' },
    { label: 'Buses visibles', value: visible, sub: 'con filtros actuales' },
    { label: 'Velocidad media', value: avgSpeed !== null ? avgSpeed + ' km/h' : '—', sub: 'de los visibles' },
    { label: 'Velocidad máx.', value: maxSpeed !== null ? maxSpeed + ' km/h' : '—', sub: '' },
    { label: 'Velocidad mín.', value: minSpeed !== null ? minSpeed + ' km/h' : '—', sub: '' }
  ];
  var row = document.createElement('div');
  row.className = 'metric-strip vertical-metrics';
  cards.forEach(function (c) {
    var div = document.createElement('div');
    div.className = 'metric-card';
    div.innerHTML = '<div class="lbl">' + esc(c.label) + '</div><div class="val">' + esc(c.value) + '</div>' + (c.sub ? '<div class="sub">' + esc(c.sub) + '</div>' : '');
    row.appendChild(div);
  });
  container.appendChild(row);

  // Gráfico de distribución de velocidades (si hay datos)
  var chartContainer = document.createElement('div');
  chartContainer.className = 'chart-wrap';
  chartContainer.style.height = '150px';
  var canvas = document.createElement('canvas');
  canvas.id = 'bus-speed-histogram';
  chartContainer.appendChild(canvas);
  container.appendChild(chartContainer);

  if (window.Chart && speeds.length > 1) {
    var bins = 10;
    var min = Math.floor(Math.min.apply(null, speeds) / 10) * 10;
    var max = Math.ceil(Math.max.apply(null, speeds) / 10) * 10;
    var step = Math.max(1, Math.ceil((max - min) / bins));
    var labels = [];
    var data = [];
    for (var i = 0; i < bins; i++) {
      ...
      data.push(speeds.filter(function (s) { return s >= lower && s < upper; }).length);
    }
    var chart = new Chart(canvas.getContext('2d'), {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [{ label: 'Buses', data: data, backgroundColor: 'rgba(152,37,28,.6)', borderRadius: 3 }]
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: function (ctx) { return ctx.parsed.y + ' buses'; } } } },
        scales: {
          x: { grid: { display: false }, ticks: { font: { size: 9 } } },
          y: { beginAtZero: true, grid: { color: 'rgba(23,32,39,.06)' }, ticks: { precision: 0 } }
        }
      }
    });
    // Guardar referencia para destruir al actualizar
    if (window._busSpeedChart) window._busSpeedChart.destroy();
    window._busSpeedChart = chart;
  }

  // Tabla resumen por operador y tipo
  var detailDiv = document.createElement('div');
  detailDiv.className = 'panel-block';
  detailDiv.innerHTML = '<span class="card-kicker">Desglose</span>';

  var opRows = Object.keys(operators).sort(function (a, b) { return operators[b] - operators[a]; });
  var opTable = '<div class="tbl-wrap"><table><thead><tr><th>Operador</th><th>Buses</th></tr></thead><tbody>' +
    opRows.map(function (op) { return '<tr><td>' + esc(op) + '</td><td>' + operators[op] + '</td></tr>'; }).join('') +
    '</tbody></table></div>';
  detailDiv.innerHTML += opTable;

  var typeRows = Object.keys(types).sort(function (a, b) { return types[b] - types[a]; });
  if (typeRows.length) {
    var typeTable = '<div class="tbl-wrap" style="margin-top:8px"><table><thead><tr><th>Tipo</th><th>Buses</th></tr></thead><tbody>' +
      typeRows.map(function (t) { return '<tr><td>' + esc(t) + '</td><td>' + types[t] + '</td></tr>'; }).join('') +
      '</tbody></table></div>';
    detailDiv.innerHTML += typeTable;
  }
...
async function applyBusFeatureLists(featureLists, sourceCount, sourceErrors) {
  ...
  updateBusRouteOptions();
  // NUEVO: inicializar filtros avanzados si no están
  initBusAdvancedFilters();
...
function switchTab(tab) {
  ...
  // NUEVO: al cambiar a la pestaña de buses, actualizar el resumen
  if (tab === 'buses') {
    updateBusSummary();
  }
}